...
class sqlClass(object):
    ...
    def insertRemoteHost(self, IP, PORT, CTYPE, USERNAME, GROUPS, NOTE, ROOTPWD, PWD=None, PKPATH=None):
        try:
            self.con.execute(
                ...
                (IP, PORT, CTYPE, USERNAME, PWD, PKPATH, GROUPS, NOTE, self.getTime(), ROOTPWD))
            self.con.commit()
        except Exception as e:
            return [False, str(e)]
        else:
    ...
    def selectRemoteHostForIP(self, IP):
        try:
            resultData = \
            self.con.execute('SELECT IP,PORT,USERNAME,PWD,ROOTPWD FROM RemoteHost WHERE IP = (?)', (IP,)).fetchall()[0]
            result = [True, resultData]
        except Exception as e:
            result = [False, str(e)]
        return result
